...
describe("simulation workbench user journeys", () => {
  ...
  it("builds and saves an environment Draft without claiming publication", async () => {
    ...
    await userEvent.click(screen.getAllByRole("button", { name: "添加点" })[0]);
    await userEvent.click(screen.getAllByRole("button", { name: "添加点" })[0]);
    await userEvent.click(screen.getAllByRole("button", { name: "添加点" })[1]);
    await userEvent.click(screen.getAllByRole("button", { name: "添加点" })[1]);
    ...
    expect(document.body.textContent).toContain("后端未提供离线作业或发布接口");
  });
  ...
  it("supports N-node scene editing with canvas and exact coordinate input", async () => {
    ...
    expect(screen.getByLabelText("节点深度分布（X 坐标）")).toBeTruthy();
  });
