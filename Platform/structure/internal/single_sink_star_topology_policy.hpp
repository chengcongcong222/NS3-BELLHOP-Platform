...
class SingleSinkStarTopologyPolicy final
...
inline auto SingleSinkStarTopologyPolicy::Build(
    const contracts::WorldSnapshot& snapshot,
    const contracts::RoleTable& role_table,
    const contracts::ConnectivityGraph& connectivity_graph) const
    -> contracts::Result<contracts::LogicalTopology> {
  const auto sink_nodes =
      role_table.NodesWithRole(contracts::ProtocolRole::kSink);
  if(sink_nodes.size() != 1) {
    return std::unexpected(
        contracts::Error{
            contracts::ErrorCode::kFailedPrecondition,
            "SingleSinkStarTopologyPolicy requires exactly one sink"});
  }
  ...
  for(const auto& edge : connectivity_graph.edges()) {
    if(edge.source_node_id == sink_node_id ||
       edge.target_node_id == sink_node_id) {
